...
_Sessions_ are used to store information about a client. It persists authentication and data across request so that the user does not need to re-enter their credentials over and over again.

2. What does bcrypt do to help us store passwords in a secure manner.

_Bcrypt_ hashes our passwords making it harder to decipher if the database were to ever get breeched.

3. What does bcrypt do to slow down attackers?

_Bcrypt_ slows down `attackers` by allowing us to hash passwords as many times as we please and will never store passwords as plain text. If attackers were to break into the database, they would need to know the hashing algorithm and how many times it has been hashed.

4. What are the three parts of the JSON Web Token?

The three parts of the _JWT_ are the `header`, the `payload`, and the `signature` all seperated by periods.
